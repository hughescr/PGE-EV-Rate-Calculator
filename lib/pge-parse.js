...
{
    let currentYear = some_date.getFullYear();
    let startDate = new Date(currentYear, Date.getMonthNumberFromName(start_date.month), start_date.day);
    let endDate = new Date(currentYear, Date.getMonthNumberFromName(end_date.month), end_date.day);
    endDate.addDays(1);
...
{
    let startTime = some_date.clone().clearTime().addHours(start_time);
    let endTime = some_date.clone().clearTime().addHours(end_time);
...
function convertToPrice(some_plan, some_rate, accumulated, time, allElectric, baselineTerritory)
{
    ...
    {
        let daysThisMonth = time.getDaysBetween(time.clone().addMonths(1));
...
exports.PGEParser = function(allElectric, baselineTerritory, wHPerMile, milesPerDay, addMilesUntilDate)
{
    ...
    this.summaryParser = function(on_completed)
    {
        ...
        let bill_parser = new xml.SaxParser(function(cb)
        {
            ...
            cb.onEndElementNS(function(elem)
            {
                ...
                if(in_intervalreading && elem == 'start')
                {
                    let date = new Date(parseInt(currentStart) * 1000);
    ...
    {
        /* These letiables track state during parsing */
        let currentDay = 0;
        let currentMonth = 0;
        let nextBill = 0;
          ...
          cb.onEndElementNS(function(elem)
          {
            if(elem == 'IntervalReading')
            {
                ...
                if(currentStart.getOrdinalNumber() != currentDay)
                {
                    ...
                    let time = currentStart.clone().clearTime();

                    for(let rateName of pge_data.rates)
                    {
